...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!isValidEmail(formData.email)) {
      showToast("Email tidak valid"); // Show a toast notification for invalid email
      return; // Exit the function if the email is not valid
    }

    if (formData.password.length < 6) {
      showToast("Password harus memiliki setidaknya 6 karakter"); // Show a toast notification for a weak password
      return; // Exit the function if the password is too short
    }

    setIsLoading(true); // Set isLoading state to true to indicate loading
    ...
    try {
      ...
      // If registration is successful, update the user's display name (fullname)
      await updateProfile(auth.currentUser, { displayName: formData.fullname });
    } catch (err) {
      console.error(err); // Log any errors to the console
      showToast(err.message); // Show a toast notification with the error message
    }

    setIsLoading(false); // Set isLoading state back to false when registration is done
  };
        ...
        <div className="flex h-screen md:flex-none xl:justify-center lg:justify-center justify-center items-center flex-wrap">
          ...
          <div className="lg:w-2/5 w-full px-4 md:px-0 items-center my-8">
            <div className="lg:px-12 md:mx-12">
              <p className="font-inter text-2xl font-bold">Sign Up to </p>
              <p className="font-inter text-sm mb-10">
                Please fill out the following information below for your account
                and email address.
              </p>
              <form className="flex flex-col gap-4" onSubmit={handleRegister}>
                <div className="flex flex-col gap-1">
                  <p className="font-inter text-xs font-semibold">Fullname</p>
                  <input
                    type="text"
                    name="fullname"
